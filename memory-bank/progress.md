...
## Recent fixes
- Storage-layer hardening alongside the `delete` fix: batch chunk inserts and wiki-index deletes (multi-statement transactions in the write worker) are now wrapped in the bounded `SQLITE_BUSY` retry, making SKILL.md's "retried automatically" claim accurate; `DeleteOrphanChunks` goes through the single-writer queue (`opDeleteOrphanChunks` via `sendWriteOpCount`) instead of bypassing it with a direct `db.Exec`; `handleDelete`/`handleGC` release storage with `defer store.Close()`.
